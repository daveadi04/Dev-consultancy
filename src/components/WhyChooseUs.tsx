...
const WhyChooseUs = () => {
  ...
  return (
    <section id="why-choose-us" className="py-12 md:py-20 bg-gradient-to-b from-gray-50 to-white">
      <div className="container mx-auto px-4">
        <div className="text-center mb-8 md:mb-16">
          ...
          <motion.div 
            className="w-24 h-1 bg-gradient-to-r from-yellow-400 to-yellow-600 mx-auto mb-4 md:mb-6"
            initial={{ width: 0 }}
          ...
          {reasons.map((reason, index) => (
            ...
            >
              <div className="flex items-start">
                <div className="mr-4 md:mr-6 p-2 md:p-3 bg-blue-50 rounded-lg">{reason.icon}</div>
                <div>
                  <h3 className="text-lg md:text-xl font-bold mb-2 md:mb-3 text-gray-900">{reason.title}</h3>
        ...
        {/* Stats Section */}
        <motion.div 
          className="bg-gradient-to-r from-blue-600 to-blue-800 rounded-xl p-6 md:p-10 shadow-xl"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.5 }}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8">
            {stats.map((stat, index) => (
              <motion.div 
                ...
                transition={{ duration: 0.5, delay: index * 0.1 }}
              >
                <div className="text-2xl md:text-4xl lg:text-5xl font-bold text-white mb-1 md:mb-2">{stat.value}</div>
                <div className="text-xs md:text-sm lg:text-base text-blue-100">{stat.label}</div>
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        {/* Client Quote */}
        <motion.div 
          className="mt-8 md:mt-16 bg-white p-6 md:p-8 rounded-xl shadow-lg text-center max-w-4xl mx-auto"
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
        ...
        >
          ...
          <blockquote className="text-lg md:text-2xl italic font-medium text-gray-700 mb-4 md:mb-6">
